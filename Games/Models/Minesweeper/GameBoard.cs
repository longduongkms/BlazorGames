...
namespace Games.Models.Minesweeper
{
    public class GameBoard
    {
        ...
        {
            get { return MineCount - Panels.Count(x => x.IsFlagged); }
        }
        ...
        private void FirstMove(int x, int y)
        {
            ...
            List<Panel> neighbors = GetNeighbors(x, y).ToList(); // Get all neighbors
            neighbors.Add(Panels.First(z => z.X == x && z.Y == y));

            //Select random panels from set which are not excluded
            IOrderedEnumerable<Panel> mines = Panels.Except(neighbors).OrderBy(_ => rand.Next());
            var mineSlots = mines.Take(MineCount).Select(z => new {z.X, z.Y});
            ...
            {
                IEnumerable<Panel> nearbyPanels = GetNeighbors(openPanel.X, openPanel.Y);
                openPanel.AdjacentMines = nearbyPanels.Count(panel => panel.IsMine);
            }
        ...
        private IEnumerable<Panel> GetNeighbors(int x, int y)
        {
            IEnumerable<Panel> nearbyPanels = Panels.Where(panel => panel.X >= x - 1 && panel.X <= x + 1
                && panel.Y >= y - 1 && panel.Y <= y + 1);
            IEnumerable<Panel> currentPanel = Panels.Where(panel => panel.X == x && panel.Y == y);

            return nearbyPanels.Except(currentPanel);
        }
        ...
        private void RevealZeros(int x, int y)
        {
            ...
            foreach (Panel neighbor in neighborPanels)
            {
        ...
        public void FlagPanel(int x, int y)
        {
            if (MinesRemaining > 0)
            {
                Panel panel = Panels.First(z => z.X == x && z.Y == y);
